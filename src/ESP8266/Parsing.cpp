...
bool ESP8266WebServer::_parseRequest(WiFiClient& client) {
  ...
  int version_start = req.indexOf('/', addr_end + 1);
  if (addr_start == -1 || addr_end == -1 || version_start == -1) {
  ...
  String protocol = req.substring(addr_end + 1, version_start);
  _currentHTCPCP = (protocol == "HTCPCP");
  String searchStr = "";
  ...
  if (methodStr == "POST") {
    ...
    method = HTTP_PATCH;
  } else if (methodStr == "BREW") {
    method = HTTP_BREW;
  } else if (methodStr == "PROPFIND") {
    method = HTTP_PROPFIND;
  } else if (methodStr == "WHEN") {
    method = HTTP_WHEN;
  }
